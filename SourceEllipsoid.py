...
def ser_dphi_elliptic(mu,zeta,U0,a,b):
    ...
    """ 
    e   = np.sqrt(abs(a**2 - b**2))/a
    A   = U0 * a /( 1/(1-e**2) - 1/(2*e) * np.log(abs((1+e)/(1-e))) )
...
def ser_u(x,y,U0,a,b):
    ...
    """
    k = a*np.sqrt(abs(a**2-b**2))/a
    mu,zeta,theta = T_cart_semielliptic(x,y,x*0,a*np.sqrt(abs(a**2-b**2))/a)
